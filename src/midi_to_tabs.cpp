...
std::vector<std::array<int, 3>> apply_range(std::vector<std::array<int, 3>> ranges, std::vector<std::pair<int, int>> midi_beats) {
    std::vector<std::array<int, 3>> notes; //струна-лад-длительность (в 16-ых долях)
    ...
    for (std::array<int, 3>& arr : ranges) {
        ...
        if (arr[0] < 0 || arr[0] > 22 || arr[1] < 0 || arr[1] > 22 || arr[0] > arr[1]) {
    ...
    for (std::pair<int, int> note : midi_beats) {
        ...
        for (std::array<int, 3> range : ranges) {
            ...
            if (note_start <= range[2]) {
                for (int st = 5; st >= 0; --st) {
                    if (note.first - strings_midi[st] >= range[0] && note.first - strings_midi[st] <= range[1]) {
        ...
        for (int st = 5; st >= 0; --st) {
            if (note.first - strings_midi[st] >= 0 && note.first - strings_midi[st] <= 22) {
                notes.push_back({st, note.first - strings_midi[st], note.second});
                goto exit;
            }
        }
...
void generate_tab(std::vector<std::array<int, 3>>& notes, std::ostream& out) {
    ...
    while (!notes.empty()) {
        ...
        if (notes[0][1] == -1) lines[notes[0][0]] << "R";
        else lines[notes[0][0]] << notes[0][1];
        
        strin = notes[0][0];
        if (len < notes[0][2]) {
            if (notes[0][1] >= 10) lines[strin] << "~~~";
            else lines[strin] << "~~~~";
            notes[0][2] -= len;
        } else {
            if (notes[0][1] >= 10) lines[strin] << "---";
